...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="msitename/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
        ...
        <img alt="正在刷新内存，请勿重复操作！" src="static/images/loading2.gif" />  
    </div>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
					...
					&nbsp;&nbsp;
				   <c:if test="${QX.add == 1 }">
					<a class="btn btn-small btn-success" id="loadData" >刷新内存</a>
					<div id="result"></div>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		$(document).ready(function () {
		    $("#loadData").bind("click", load);
		});
		function load(){
	        htmlobj = $.ajax({
	            type: "GET",
	            dataType: "html",
	            url: "<%=basePath%>load/update.do",
	            data: {}, //参数信息，采用ＪＳ对象的形式，也可以使用URL地址比较传统的&将参数分隔
	            error: function () {
	                alert("刷新内存数据失败");
	            },
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>msitename/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>msitename/delete.do?id="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>msitename/goEdit.do?id='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>msitename/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>msitename/excel.do';
		}
		</script>
